...
chcp 65001 > nul
REM ============================================================
REM  Pandora - Build and Cook Linux Dedicated Server
...
if "%LINUX_MULTIARCH_ROOT%"=="" (
    echo [Pandora] WARNING: LINUX_MULTIARCH_ROOT not set.
...
echo [Pandora] LINUX_MULTIARCH_ROOT=%LINUX_MULTIARCH_ROOT%
echo [Pandora] Building Linux Dedicated Server (%CONFIG%)...
...
if errorlevel 1 (
    echo [Pandora] ERROR: Linux server build failed.
    pause
    exit /b 1
)

echo.
echo [Pandora] Linux Server build complete.
echo Output: %OUTPUT_DIR%\LinuxServer\
echo.
echo Deploy to Linux:
echo   scp -r %OUTPUT_DIR%\LinuxServer user@host:/opt/pandora/
echo   ssh user@host
echo   cd /opt/pandora/LinuxServer
echo   chmod +x PandoraServer.sh
echo   ./PandoraServer.sh -log -port=7777
echo.
pause