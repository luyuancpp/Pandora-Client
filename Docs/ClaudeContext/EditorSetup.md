...
1. **在 `Content/` 下右键创建蓝图**：
   ...
   - `BP_XuanmingWeapon_AK` 继承 `XuanmingWeapon`（脚本 `Tools/CreateWeaponBP.py` 已建）
...
3. **配置默认值**：
   ...
   - `Project Settings → Maps & Modes → GlobalDefaultServerGameMode = BP_XuanmingGameMode`（**别忘**，否则真 DS 模式 IA 字段全 nullptr）
...
1. 启用 `ModelViewViewModel` 插件（`Xuanming.uproject` 已开）
2. 用 Designer 在 `Content/UI/` 手搭 `WBP_PlayerHUD`：
   ...
   - `Window → View Bindings`：
     - 创建 ViewModel，类型 `UXuanmingPlayerViewModel`，名称 `PlayerVM`
     - HealthBar.Percent ↔ PlayerVM.GetHealthPercent()
     - HealthText.Text ↔ PlayerVM.GetHealthText()
     - AmmoText.Text ↔ PlayerVM.GetAmmoText()
3. `BP_XuanmingPlayerController.HUDWidgetClass = WBP_PlayerHUD`
...
2. **Project Settings → GameplayTags 注册**：
   - `State.Debuff.Slow`
   - `Cooldown.FrostCurse`
   - 或开 `bAllowGameplayTagsCreationByDefault=true` 让 GE 构造时自动注册（**不推荐**，CI/team 拉代码 tag 表会漂）
3. IMC_Default 加 `IA_FrostCurse`（bool, Q 键）
4. BP_XuanmingCharacter 指派：
   - `IA_FrostCurse = IA_FrostCurse`
   - `StartupAbilities[0] = UXuanmingGA_FrostCurse::StaticClass()`（或 BP 子类）
