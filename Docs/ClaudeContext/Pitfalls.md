...
| AttributeSet 改 MoveSpeed 不会自动同步到 CharacterMovement | `FGameplayAttributeData` 只是数字，引擎不知道你想干啥。在 `PostGameplayEffectExecute` 里手动 `Movement->MaxWalkSpeed = BaseSpeed * GetMoveSpeed()`，否则冰咒 GE 应用了但角��照常跑 |
