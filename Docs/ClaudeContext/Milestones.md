...
### M1.5 (2026-06-02)：GAS 框架 + 玄冥冰咒示例技能 ✅ 闭环
...
- ✅ `AXuanmingCharacter::BindAttributeDelegates` 订阅 `OnGameplayAttributeValueChange(MoveSpeed)` → 同步��� `CharacterMovement->MaxWalkSpeed`（**Duration GE 不触发 PostGameplayEffectExecute**，必须走这条委托链路）
- ✅ 编辑器配置完成：
  ...
  - `BP_XuanmingCharacter` 指派 `IA_FrostCurse` + `StartupAbilities[0]=BP_GA_FrostCurse`
  - `Content/Maps/Sandbox/L_Sandbox_01.umap` 新建测试关（含 PlayerStart + Dummy BP_XuanmingCharacter）
...
- ⏳ DS + 2 Client 验证留 M1.x：需先 `BuildServer.bat` 重编 server（`XuanmingServer.exe` 改 UPROPERTY 后必须重编，详见 `Pitfalls.md`）
