...
#include "PandoraWeapon.generated.h"

class UStaticMeshComponent;
class APandoraCharacter;

/**
 * Pandora武器基类 - 服务器权威 hitscan 射击
 *
 * 流程：
 *   客户端按下开火 → StartFire() →
 ...
 */
UCLASS()
class PANDORA_API APandoraWeapon : public AActor
{
	GENERATED_BODY()
...
public:
	...
	UPROPERTY(VisibleAnywhere, BlueprintReadOnly, Category = "Pandora|Weapon")
	TObjectPtr<UStaticMeshComponent> WeaponMesh;

	/** 单发伤害 */
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category = "Pandora|Weapon|Stats")
	float Damage = 25.0f;
	...
	/** 当前弹匣余量（服务器权威） */
	UPROPERTY(Replicated, BlueprintReadOnly, Category = "Pandora|Weapon|Stats")
	...
	UFUNCTION(BlueprintCallable, Category = "Pandora|Weapon")
	void StartFire();
...
protected:
	...
	/** 取得拥有者角色（由 Owner 转型） */
	APandoraCharacter* GetOwnerCharacter() const;
