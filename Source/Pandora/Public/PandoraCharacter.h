...
#include "PandoraCharacter.generated.h"

class UCameraComponent;
class UInputAction;
class UInputMappingContext;
class APandoraWeapon;
...
/**
 * Pandora Character - FPS 玩家角色
 * 包含输入处理、相机、武器持有、服务器权威血量与伤害
 */
UCLASS()
class PANDORA_API APandoraCharacter : public ACharacter, public IAbilitySystemInterface
{
	GENERATED_BODY()
...
public:
	...
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category = "Pandora|GAS")
	...
	// === 状态 ===
	UPROPERTY(ReplicatedUsing = OnRep_Health, BlueprintReadOnly, Category = "Pandora|Combat")
	float Health = 100.0f;

	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category = "Pandora|Combat")
	float MaxHealth = 100.0f;

	// === 武器 ===
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category = "Pandora|Weapon")
	TSubclassOf<APandoraWeapon> DefaultWeaponClass;

	UPROPERTY(Replicated, BlueprintReadOnly, Category = "Pandora|Weapon")
	TObjectPtr<APandoraWeapon> CurrentWeapon;

	UFUNCTION()
	void OnRep_Health();

	/** 由武器查询：相机/眼睛位置，用于射线起点 */
	UFUNCTION(BlueprintCallable, Category = "Pandora|Combat")
...
protected:
	...
	UPROPERTY(VisibleAnywhere, BlueprintReadOnly, Category = "Pandora|Camera")
...
public:
	/**
	 * 调试用自伤命令: PIE 控制台直接输入 "PandoraDamageSelf 50" 即可对自己造成 50 伤害.
	 * Exec 关键字让函数自动注册为控制台命令.
	 * 仅本地客户端调用, 内部走 ServerRPC 让 server 真实扣血.
	 */
	UFUNCTION(Exec)
	void PandoraDamageSelf(float Amount = 25.f);
