...
APandoraPlayerController::APandoraPlayerController()
{
}

void APandoraPlayerController::BeginPlay()
{
	Super::BeginPlay();
	UE_LOG(LogTemp, Warning, TEXT("[Pandora] PlayerController BeginPlay (IsLocal=%d)"), IsLocalController());
...
void APandoraPlayerController::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
	if (HUDWidget)
	{
...
void APandoraPlayerController::PlayerTick(float DeltaTime)
{
...
void APandoraPlayerController::AcknowledgePossession(APawn* P)
{
	Super::AcknowledgePossession(P);
	// 客户端收到 server 通知 possess 完成 = LocalPlayer 一定就绪了
	// DS+Client 模式 IMC / HUD 真正在这里注册成功
	UE_LOG(LogTemp, Warning, TEXT("[Pandora] PlayerController AcknowledgePossession Pawn=%s"),
		*GetNameSafe(P));
...
void APandoraPlayerController::OnPossess(APawn* InPawn)
{
...
void APandoraPlayerController::ApplyFPSInputMode()
{
	// FPS 输入模式 (幂等可重复调用):
	...
	UE_LOG(LogTemp, Warning, TEXT("[Pandora] FPS InputMode applied"));
	...
	{
		UE_LOG(LogTemp, Error, TEXT("[Pandora] FPS InputMode: GameViewport=null"));
	}
...
void APandoraPlayerController::TryRegisterIMC(const TCHAR* From)
{
	if (bIMCRegistered)
	{
		return;
	...
	if (!LP)
	{
		UE_LOG(LogTemp, Warning, TEXT("[Pandora] IMC 注册推迟(%s): LocalPlayer 未就绪"), From);
		return;
	}
	...
	if (!Subsystem)
	{
		UE_LOG(LogTemp, Warning, TEXT("[Pandora] IMC 注册失败(%s): EnhancedInputSubsystem 未就绪"), From);
		return;
	}
	Subsystem->AddMappingContext(DefaultMappingContext, 0);
	bIMCRegistered = true;
	UE_LOG(LogTemp, Warning, TEXT("[Pandora] IMC 注册成功(%s): %s"),
		From, *DefaultMappingContext->GetName());
}

void APandoraPlayerController::TryCreateHUD(const TCHAR* From)
{
	...
	if (!HUDWidgetClass)
	{
		UE_LOG(LogTemp, Warning, TEXT("[Pandora] HUD 创建跳过(%s): HUDWidgetClass 未指派 (BP_PandoraPlayerController.HUDWidgetClass)"),
			From);
		return;
	}
	...
	if (!GetLocalPlayer())
	{
		UE_LOG(LogTemp, Warning, TEXT("[Pandora] HUD 创建推迟(%s): LocalPlayer 未就绪"), From);
		return;
	}
	...
	if (!HUDWidget)
	{
		UE_LOG(LogTemp, Error, TEXT("[Pandora] HUD 创建失败(%s): CreateWidget 返回 null"), From);
		return;
	}
	HUDWidget->AddToViewport(0);
	UE_LOG(LogTemp, Warning, TEXT("[Pandora] HUD 创建成功(%s): %s"),
		From, *HUDWidgetClass->GetName());

	// 立即创建 ViewModel 并尝试注入 (WBP MVVM View 在 NativeConstruct 时已就绪)
	if (!PlayerViewModel)
	{
		PlayerViewModel = NewObject<UPandoraPlayerViewModel>(this);
		UE_LOG(LogTemp, Warning, TEXT("[Pandora] PlayerViewModel 创建完成"));
	}
	TryInjectViewModel(From);
}

bool APandoraPlayerController::TryInjectViewModel(const TCHAR* From)
{
	...
	if (!View)
	{
		...
		UE_LOG(LogTemp, Warning,
			TEXT("[Pandora] ViewModel 注入跳过(%s): WBP_PlayerHUD 未配 MVVM Bindings (Window->View Bindings)"),
			From);
	...
	{
		UE_LOG(LogTemp, Warning,
			TEXT("[Pandora] ViewModel 注入失败(%s): SetViewModel(%s) 返回 false. 检查 WBP 里 ViewModel Name 是否匹配, Creation Type 是否为 Manual"),
	...
	bViewModelInjected = true;
	UE_LOG(LogTemp, Warning, TEXT("[Pandora] ViewModel 注入成功(%s): %s"),
		From, *PlayerViewModelName.ToString());
	return true;
}

void APandoraPlayerController::PushStateToViewModel()
{
	if (!PlayerViewModel)
	{
		return;
	...
	APandoraCharacter* Ch = Cast<APandoraCharacter>(GetPawn());
	if (!Ch)
	{
		return;
	}
	...
	if (APandoraWeapon* W = Ch->CurrentWeapon)
	{
