...
chcp 65001 > nul
REM ============================================================
REM  Pandora - Generate Visual Studio Project Files
REM
...
set PROJECT_FILE=%PROJECT_ROOT%Pandora.uproject
...
echo [Pandora] Step 1/4: Checking that engine source is untouched...
call "%PROJECT_ROOT%Tools\CheckEngineUntouched.bat" || (
    echo [Pandora] ABORT: engine has unexpected modifications.
    pause
    exit /b 1
)

REM ---- Step 2: locate .NET 8 SDK ----
echo [Pandora] Step 2/4: Locating .NET 8 SDK...
where dotnet >nul 2>&1 || (
    echo [Pandora] ERROR: dotnet not found in PATH.
...
echo [Pandora]   project-pinned .NET version: %DOTNET_VERSION%
REM Check version starts with "8." -- avoid findstr regex pitfalls in delayed expansion
if not "%DOTNET_VERSION:~0,2%"=="8." (
    echo [Pandora] ERROR: project requires .NET 8, got %DOTNET_VERSION%.
...
echo [Pandora] Step 3/4: Verifying UnrealBuildTool is built...
if not exist "%UBT_DLL%" (
    echo [Pandora]   UBT dll missing, building from source with .NET 8...
    ...
    popd
    if not "%BUILD_RC%"=="0" (
        echo [Pandora] ERROR: UBT build failed.
        pause
        exit /b 1
    )
)
...
echo [Pandora] Step 4/4: Generating Visual Studio %VS_VERSION% project files...
pushd "%PROJECT_ROOT%"
...
if not "%GEN_RC%"=="0" (
    echo [Pandora] ERROR: Failed to generate project files.
    pause
    exit /b 1
)

echo.
echo [Pandora] Done. Open Pandora.sln in Visual Studio.
pause